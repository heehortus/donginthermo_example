...
export default function ProductDetailPage() {
  const { category: categoryParam, product: productParam } = useParams<{ category: string; product: string }>()
  const category = categoryParam as CategoryId
  if (!categories.includes(category)) notFound()

  const product = getProduct(category, productParam)
  if (!product) notFound()
  ...
  const siblings = getProductsByCategory(category).filter((p) => p.id !== product.id)
